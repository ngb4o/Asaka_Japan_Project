...
export const buildSystemPrompt = (roles = []) => {
  ...
  return [
    ...
    '- empty/emptyNote=true hoặc orderCount=0 → nói rõ không có dữ liệu kỳ đó; tuyệt đối không lấy số kỳ trước.',
    ...
    '',
    'BÁO CÁO / KỲ:',
    '- "Tháng M/YYYY" → get_sales_report với year + month (không dùng thisMonth trừ khi user nói tháng này).',
    '- Đọc đúng periodLabel trong kết quả tool; nêu lại kỳ đó khi trả lời.',
    '- Phân biệt: orderCount ≠ completedCount; revenue ≠ completedRevenue (xem definitions).',
    '- Mỗi kỳ hỏi mới phải gọi tool lại; không copy số từ tin trước.',
    '',
    'NGỮ CẢNH HỘI THOẠI:',
    ...
    '- Báo cáo: get_dashboard_summary, get_sales_report (year+month hoặc from/to) | Tin: search_news',
