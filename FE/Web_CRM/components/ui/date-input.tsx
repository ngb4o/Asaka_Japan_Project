...
/**
 * Polished date field: dd/mm/yyyy + calendar popover.
 * Value API uses ISO `yyyy-mm-dd`.
 */
export const DateInput = React.forwardRef<HTMLInputElement, DateInputProps>(
  (
    ...
    const isoValue = toDateValue(value);
    const selectedDate = isoValue ? parseIsoLocal(isoValue) : null;
    const [open, setOpen] = React.useState(false);
    const [text, setText] = React.useState(() =>
      isoValue ? formatDateDisplay(isoValue) : ""
    );
    ...
    React.useEffect(() => {
      setText(isoValue ? formatDateDisplay(isoValue) : "");
      if (selectedDate) setViewMonth(startOfMonth(selectedDate));
    }, [isoValue]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className={cn("relative w-full", className)}>
          ...
          >
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-sm tabular-nums tracking-wide text-[var(--color-text-primary)] outline-none placeholder:tracking-normal placeholder:text-[var(--color-text-inverse)] disabled:cursor-not-allowed"
              {...props}
            />

            <div className="flex shrink-0 items-center gap-0.5">
              {clearable && isoValue && !disabled ? (
                <button
                  type="button"
                  aria-label="Xóa ngày"
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        !isSelected && inMonth && "hover:bg-[var(--color-surface-muted)]",
                        isSelected &&
