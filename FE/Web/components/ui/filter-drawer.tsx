...
function FilterFooter({
  ...
  return (
    <div
      ...
      <Button type="button" size="sm" className="text-white" onClick={onDone}>
        Xong
      </Button>
    </div>
  );
  ...
  if (isMobile) {
    return (
      <BottomSheet
        ...
        maxHeight="85dvh"
        className={className}>
        <FilterPanelBody canClear={draftCount > 0} onClear={onClear} onDone={onApply}>
          {children}
        </FilterPanelBody>
      </BottomSheet>
    );
    ...
    <div
      ...
      <aside
        ...
        <div className="shrink-0 border-t border-[var(--color-border-subtle)] p-4">
          <FilterFooter canClear={draftCount > 0} onClear={onClear} onDone={onApply} />
        </div>
      </aside>
    </div>,
    document.body
